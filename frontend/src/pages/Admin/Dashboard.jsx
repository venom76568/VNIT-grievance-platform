...
export default function AdminDashboard() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await axios.put(
        `${API}/admin/complaints/${selectedComplaint.id}/approve`,
        formData
      );
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await axios.put(
        `${API}/admin/complaints/${selectedComplaint.id}/reject`,
        formData
      );
  ...
  const filterComplaints = (status) => {
    ...
    if (status === "review")
      return complaints.filter(
        (c) => c.status === "Completed - Awaiting Admin Review"
      );
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <Tabs defaultValue="pending" className="space-y-6">
          ...
          {["pending", "assigned", "review", "completed", "rejected"].map(
            (tab) => (
              <TabsContent
                key={tab}
                value={tab === "review" ? "review" : tab.toLowerCase()}
              >
                        ...
                        <div className="flex items-center space-x-6 text-sm text-gray-600">
                          ...
                          {complaint.count > 1 && (
                            <span className="font-semibold text-blue-600">
                              {complaint.count} similar complaints
                            </span>
                          )}
                          <span>
                            {new Date(
