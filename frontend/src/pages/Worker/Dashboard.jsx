...
const statusColors = {
  ...
  'Completed - Awaiting Admin Review': 'bg-green-100 text-green-800 border-green-200',
...
export default function WorkerDashboard() {
  ...
  const handleUpdateStatus = async (status) => {
    try {
      ...
      await axios.put(`${API}/worker/tasks/${selectedTask.id}/status`, formData);
  ...
  const pendingTasks = tasks.filter(t => ['Assigned', 'In Progress', 'RequestedChanges'].includes(t.status));
  const completedTasks = tasks.filter(t => ['Resolved', 'Cannot be Resolved', 'Completed - Awaiting Admin Review', 'Completed'].includes(t.status));
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              <div className="text-right hidden sm:block">
                ...
                <p className="text-xs text-gray-600 capitalize">{user?.specialization}</p>
              </div>
              <Button
                onClick={logout}
                variant="outline"
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-4xl font-bold text-blue-600 mt-2">{pendingTasks.length}</p>
              </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-4xl font-bold text-green-600 mt-2">{completedTasks.length}</p>
              </div>
        ...
        <Tabs defaultValue="pending" className="space-y-6">
          <TabsList className="bg-white rounded-xl shadow-md p-1 border border-gray-100">
            <TabsTrigger value="pending" className="rounded-lg" data-testid="pending-tasks-tab">
              Pending ({pendingTasks.length})
            </TabsTrigger>
            <TabsTrigger value="completed" className="rounded-lg" data-testid="completed-tasks-tab">
          ...
          <TabsContent value="pending">
            <div className="grid gap-6">
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-xl font-bold text-gray-900">{task.category}</h3>
                        <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${statusColors[task.status] || 'bg-gray-100 text-gray-800'}`}>
                          {task.status}
                        </span>
                      ...
                      <p className="text-gray-600 text-sm mb-1">{task.subcategory}</p>
                  ...
                  <div className="flex space-x-3">
                    ...
                    <Button
                      onClick={() => {
                        setSelectedTask(task);
                        setActionDialog({ open: true, action: 'Cannot be Resolved' });
                      }}
                      variant="outline"
              ...
              {pendingTasks.length === 0 && (
                <div className="text-center py-16 bg-white rounded-2xl shadow-lg" data-testid="no-pending-tasks">
                  <Wrench className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">No pending tasks</h3>
          ...
          <TabsContent value="completed">
            <div className="grid gap-6">
                    ...
                    <div className="flex-1">
                      ...
                      {task.resolution && (
                        <div className="mt-3 bg-gray-50 border border-gray-200 rounded-lg p-3">
                          <p className="text-sm text-gray-700 font-semibold">Resolution:</p>
                          <p className="text-sm text-gray-600 mt-1">{task.resolution}</p>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {completedTasks.length === 0 && (
                <div className="text-center py-16 bg-white rounded-2xl shadow-lg" data-testid="no-completed-tasks">
        ...
        <DialogContent className="sm:max-w-[500px] rounded-2xl">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold">
              {actionDialog.action === 'Resolved' ? 'Mark Task as Resolved' : 'Mark Task as Cannot be Resolved'}
            </DialogTitle>
          </DialogHeader>
